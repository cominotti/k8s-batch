...
class RulesEngineDroolsIT extends AbstractStandaloneBatchTest {
    ...
    @Test
    void shouldRecordDroolsAsEngineForAllRows() throws Exception {
        String inputFile = testResourcePath("test-data/csv/rules-poc/financial-transactions.csv");
        jobOperatorTestUtils.startJob(rulesEngineJobParams(inputFile));

        Integer droolsCount = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM rules_poc_enriched_transactions WHERE rules_engine = 'drools'",
                Integer.class);
        assertThat(droolsCount).isEqualTo(10);
    }
